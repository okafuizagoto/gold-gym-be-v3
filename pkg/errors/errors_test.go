...
func TestWrapf(t *testing.T) {
	...
	for i, tt := range tests {
		...
		// }
		var got string
		// Menggunakan string literal untuk format string agar sesuai dengan requirement Go compiler
		switch i {
		case 0:
			got = Wrapf(tt.err, "read error").Error()
		case 1:
			got = Wrapf(tt.err, "client error").Error()
		case 2:
			got = Wrapf(tt.err, "client error").Error()
		}
		if got != tt.want {
			t.Errorf("Wrapf(%v): got: %v, want %v", tt.err, got, tt.want)
		}
	}
}
...
func TestWithMessagef(t *testing.T) {
	...
	for i, tt := range tests {
		...
		switch i {
		case 0:
			got = WithMessagef(tt.err, "read error").Error()
		case 1:
			got = WithMessagef(tt.err, "client error").Error()
		case 2:
			got = WithMessagef(tt.err, "client error").Error()
		}
		if got != tt.want {
			t.Errorf("WithMessagef(%v): got: %q, want %q", tt.err, got, tt.want)
		}
	}
}
